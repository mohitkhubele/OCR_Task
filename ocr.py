...
input_file = "F:/internshala/ocr_task/input"
def ii(xx, yy):
...
for img_path in glob.glob(input_file+"/*.jpg"):
    # Load the image
    ...
    cv2.imwrite(img_path.replace("input","output"), new_image)
    raw_data=pytesseract.image_to_string(new_image)
    f = open((img_path.replace("input","output")).replace(".jpg",".txt"), "a")
    f.write(raw_data)
    f.close()
